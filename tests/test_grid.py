...
def test_get_results(test_system): 
    """Check if getting out results work. """
    var_names = ("n:fehz:bus1","m:P:bus1","s:firel", "s:outofstep")  
    output = test_system.generate_variables(var_names)
...
def test_get_rating(test_system): 
    ...
    assert test_system.get_ratings()[0] == pytest.approx(37.24, abs = 0.01)
...
def test_power_flow_calc(test_system): 
    LDF = test_system.power_flow_calc() 
    assert LDF.PrintCheckResults() == 1  

def test_power_power_calc_converged(test_system): 
    assert test_system.power_calc_converged() == True
...
def test_get_machines_inertia_list(test_system): 
    ...
    assert float(inertia_list[0][1]) == pytest.approx(11.34, abs = 0.01)

def test_get_inertia(test_system): 
    assert test_system.get_inertia("SM1") == pytest.approx(11.34, abs = 0.01)
